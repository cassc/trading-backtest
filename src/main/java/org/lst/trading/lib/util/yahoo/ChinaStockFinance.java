...
public class ChinaStockFinance implements HistoricalPriceService {
    ...
    public static final CsvReader.ParseFunction<Double> ADJ_COLUMN = doubleColumn("close");
    ...
    private static final Logger log = LoggerFactory.getLogger(ChinaStockFinance.class);
    ...
    public Observable<DoubleSeries> getHistoricalAdjustedPrices(String symbol, Instant from, Instant to) {
        return getHistoricalPricesCsv(symbol, from, to).map(csv -> tencentToDoubleSeries(csv, symbol));
    }
    ...
    private static DoubleSeries tencentToDoubleSeries(String tencentJsFrag, String symbol) {
        String json = tencentJsFrag.split("=")[1];
        JSONObject dataNode = (JSONObject) new JSONObject(json).get("data");
        JSONArray kdNodes = (JSONArray) ((JSONObject) dataNode.get(symbol)).get("qfqday");
        StringBuilder sb = new StringBuilder();
        sb.append("date,open,close,high,low,volume");
        for (Object kd : kdNodes) {
            JSONArray kdArr = (JSONArray) kd;
            for (Object item : kdArr) {
...
//        return format("http://ichart.yahoo.com/table.csv?s=%s&%s&%s&g=d&ignore=.csv", symbol, toYahooQueryDate(from, "abc"), toYahooQueryDate(to, "def"));
        return format("http://web.ifzq.gtimg.cn/appstock/app/fqkline/get?_var=kline_dayqfq2017&param=%s,day,2017-01-01,2018-12-31,640,qfq&r=0.20046413623249008", symbol);
    }
